...
| `MAX_TOKENS_DEFAULT` | `8192` | 客户端未传 max_tokens 时注入的默认值 |

### 小模型过滤

控制台可设置「最小参数量」（默认 0 = 不过滤）。auto 模式下，参数量低于阈值的
模型不参与调度——9B 级小模型在智能体场景里思考慢、能力弱，纯属浪费配额。

- 参数量从模型 ID 解析（如 `glm-z1-9b-free` → 9B、`gpt-oss-20b` → 20B、
  MoE 总参优先于激活参数、支持 `8x7b` 乘法格式）
- 无法解析参数量的模型一律保留（不误杀）
- **点名指定模型不受影响**——显式请求视为明确意图

### 免费模型识别与排除

OpenAI 协议的 `/v1/models` **不携带任何计费信息**，所以网关用两层方案：

1. **free 标记识别**：控制台勾选「仅调度带 free 标记的模型」后，auto 只调度 ID 中含 `free`（`-free` / `:free` / `_free` 等形态）的模型。
2. **手动排除清单**：「模型调度控制」卡片列出全部合并模型（支持搜索），点「排除」把收费模型拉黑，黑名单持久化。被排除的模型不参与 auto 调度，点名指定仍可用。

> 注意：回复 JSON 里的 `model` 字段是上游回显的名字，部分上游会剥掉 `-free` 后缀；网关实际调度的模型以 `gateway.log` 的记录为准。
